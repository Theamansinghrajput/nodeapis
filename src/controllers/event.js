...
exports.store = async (req, res) => {
    try {
        ...
        if (!req.file) return res.status(200).json({event, message: 'Productadded successfully'});

        //Attempt to upload to cloudinary
        const result = await uploader(req);
        const event_ = await Event.findByIdAndUpdate(event._id, {$set: {image: result.url}}, {new: true});

        res.status(200).json({event: event_, message: 'Productadded successfully'});
    } catch (error) {
...
exports.show = async function (req, res) {
    try {
        ...
        if (!event) return res.status(401).json({message: 'Productdoes not exist'});
...
exports.update = async function (req, res) {
    try {
        ...
        //if there is no image, return success message
        if (!req.file) return res.status(200).json({event, message: 'Producthas been updated'});

        //Attempt to upload to cloudinary
        const result = await uploader(req);
        const event_ = await Event.findOneAndUpdate({_id: id, userId}, {$set: {image: result.url}}, {new: true});

        res.status(200).json({event: event_, message: 'Producthas been updated'});
    } catch (error) {
        res.status(500).json({message: error.message});
    }
};
...
exports.destroy = async function (req, res) {
    try {
        ...
        res.status(200).json({message: 'Producthas been deleted'});
    } catch (error) {
        res.status(500).json({message: error.message});
    }
};
